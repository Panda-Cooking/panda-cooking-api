...
describe("Update images on recipe route tests", () => {
    let conn: DataSource;
    ...
    test("PATCH /recipes/:id/imagesrecipes/:id - Should be able to update image on recipe", async () => {
        ...
        const response = await request(app)
            .patch(`${baseUrl}/${recipeId}/imagesrecipes/${imageRecipeId}`)
            .set("Authorization", `Bearer ${userLogin.body.token}`)
    ...
    test("PATCH /recipes/:id/imagesrecipes/:id - Should not be able to update image on recipe without authorization", async () => {
        const responseAllRecipes = await request(app).get(baseUrl).send();
    ...
    test("PATCH /recipes/:id/imagesrecipes/:id - Should not be able to update image on recipe with invalid recipeId", async () => {
        const userLogin = await request(app)
            .post("/auth")
            .send(mockedUserLoginRequest);
    ...
    test("PATCH /recipes/:id/imagesrecipes/:id - Should not be able to update image on recipe with invalid imageRecipeId", async () => {
        const userLogin = await request(app)
            .post("/auth")
            .send(mockedUserLoginRequest);
